...
exports.getCandidatesApplications = async function getCandidatesApplications(req, res, moreThanMetricsDB) {
    ...
    const getCandidateApplications =
        "SELECT application_id, jobs.job_id, job_title, company_name, reviewed, accepted FROM application_status JOIN jobs ON jobs.job_id = application_status.job_id JOIN companies ON companies.company_id = jobs.company_id WHERE candidate_id = $1 ORDER BY application_id DESC"
